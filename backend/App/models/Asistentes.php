...
sql;
  ...
  public static function getDataUserByEmailOrName($search)
  {
    $mysqli = Database::getInstance();
    $query = <<<sql
      SELECT *
      FROM utilerias_administradores WHERE
      usuario = '$search' or nombre = '$search'
sql;
...
sql;
  ...
  public static function getNombreUser($user_id)
  {
    $mysqli = Database::getInstance();
    $query = <<<sql
    SELECT *,concat_ws(' ',nombre,apellidop,apellidom) as nombre_completo from utilerias_administradores WHERE user_id = '$user_id' or usuario = '$user_id' or  CONCAT_WS(' ',nombre,apellidop,apellidom) LIKE '%$user_id'
sql;
