...
html;
    ...
    public function getAllColaboradoresAsignadosByName($name){
        ...
        foreach (GeneralDao::getAllColaboradoresCongreso($name) as $key => $value) {
            $incremento++;
            $clave_socio = '';
            $clave_beca = '';
            $clave_beca_2 = '';
...
html;
            ...
            $status_pendiente = GeneralDao::getStatusValidandoCongreso($value['user_id']);
...
html;
            ...
            if($status_pendiente){
                if($status_pendiente['url_archivo'] != ''){
                $status3 .= <<<html
...
html;
                ...
                $status3 .= <<<html
                <span class="badge badge-success" style="background-color:#F2B500; color:#FFF !important;"><strong>Sin Comprobante</strong></span>       
    html;
                }
            }
